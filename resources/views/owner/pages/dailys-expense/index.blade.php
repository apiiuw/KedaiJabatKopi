      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
         ...
         <div class="flex flex-row-reverse items-center justify-between h-32 bg-greenJagat px-6 rounded-md">
            ...
            <div class="flex flex-col items-start">
               <h1 class="text-white font-semibold text-xl">Total Daily's Expense</h1>
               <p class="text-2xl text-white counter" data-target="{{ $totalDailyExpenseAll }}">0</p>
            </div>
         </div>
         ...
         <div class="flex flex-row-reverse items-center justify-between h-32 bg-greenJagat px-6 rounded-md">
            ...
            <div class="flex flex-col items-start">
               <h1 class="text-white font-semibold text-xl">Total Daily's Amount Expense</h1>
               <p class="text-2xl text-white counter" data-target="{{ $totalDailyAmountAll }}" data-currency="idr"></p>
            </div>
         </div>

      </div>

      <form method="GET" action="{{ route('owner.dailys-expense') }}" class="mb-4">
         <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
            ...
            <div class="flex items-end gap-3">
               ...
               {{-- Add Menu --}}
               <a href="{{ route('owner.dailys-expense.add-expense') }}"
         ...
         <table class="w-full text-md text-left rtl:text-right text-greenJagat">
            <thead class="text-md text-greenJagat uppercase bg-lightGreenJagat">
                  <tr>
                     ...
                     <th scope="col" class="px-6 py-3">
                        <a href="{{ route('owner.dailys-expense', array_merge(request()->all(), [
                           'sort' => 'created_at',
            ...
            <tbody>
               @forelse ($expenses as $expense)
                  <tr class="bg-white border-b border-gray-200 hover:bg-[#F3F7F5]">
                        ...
                        <td class="px-6 py-4">
                           {{ \Carbon\Carbon::parse($expense->created_at)->format('H.i') }}
                        </td>
                        <td class="px-6 py-4">
                           {{ $expense->category ?? '-' }}
                        </td>
                        ...
                        <td class="px-6 py-4 text-right font-calistoga">
                           <a href="{{ route('owner.dailys-expense.edit', $expense->id) }}"
            ...
            <tbody>
                  <tr class="bg-white border border-gray-300">
                     ...
                     <td class="px-6 py-4 text-end w-3/12">
                        Rp {{ number_format($totalDailyAmount, 0, ',', '.') }}
                     </td>
                  </tr>
            </tbody>
         </table>
