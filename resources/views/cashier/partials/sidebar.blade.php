...
<aside id="separator-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
   ...
   <div class="h-full px-3 py-4 overflow-y-auto bg-[#F3F7F5]">
      <ul class="space-y-2 font-medium">
         ...
         <li>
            <a href="{{ route('cashier.manage-menu') }}" class="{{ request()->is('cashier/manage-menu') || request()->is('cashier/manage-menu/add-menu') ? 'bg-greenJagat text-white' : 'hover:bg-lightGreenJagat' }} flex items-center p-2 text-greenJagat rounded-lg group transition duration-500 ease-in-out">
               <svg class="w-5 h-5 text-greenJagat transition duration-500 ease-in-out {{ request()->is('cashier/manage-menu') || request()->is('cashier/manage-menu/add-menu') ? 'text-white' : '' }}" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 18 18">
